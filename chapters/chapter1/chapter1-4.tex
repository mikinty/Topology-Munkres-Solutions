...
\bx{
  \ea{
    ...
    \item Proving the basic properties \begin{enumerate}[label=(\arabic*)]
      \item $1 > 0$ and $1 \in \mathbb{Z}$, so $1 \in \mathbb{Z}_+$. Now for any
      $x \in \mathbb{Z}_+$, the number $x+1$ is still an integer, and since $x >
      0$, we know $x + 1 > 0$, which means $x + 1 \in \mathbb{Z}_+$ as well.
      Therefore, we conclude $\mathbb{Z}_+$ is inductive.

      \item \AFSOC $A \neq \mathbb{Z}_+$. Then we know that $\exists x \in
...
\bx{
  \ea{
    \item We have $a \in \mathbb{Z}_+$. We just want to inductively show that $a + x \in \mathbb{Z}_+, \forall x \in \mathbb{Z}_+$.
